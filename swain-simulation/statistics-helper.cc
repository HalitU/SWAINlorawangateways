...
void StatisticsHelper::FillAverageTXInformationFile (
    ...
    // Other configs
    gv->averageTxInformationFile << GlobalVariables::txDutyDuration << "\t";
    gv->averageTxInformationFile << GlobalVariables::txDutyCycleBackoff;
    ...
    GlobalVariables *gv, NodeContainer gateways, NodeContainer endDevices, LoraPacketTracker &tracker)
{
    ...
    for (NodeContainer::Iterator i = endDevices.Begin (); i != endDevices.End (); ++i)
    {
        ...
        if (node->visitedOnce == true) {
            // Name
            gv->perSensorResultsFile << node->node_name << "\t";

            // Clustering parameters
            gv->perSensorResultsFile << GlobalVariables::distance_threshold << "\t";
            gv->perSensorResultsFile << GlobalVariables::outlier_threshold << "\t";
            gv->perSensorResultsFile << GlobalVariables::remove_intersections << "\t";
            gv->perSensorResultsFile << GlobalVariables::score_threshold << "\t";
            ...
            for (NodeContainer::Iterator i = gateways.Begin (); i != gateways.End (); ++i) {
                ...
                double currentDistance = SensorTopologyManager::CalculateDistanceBetweenNodes (current_gateway, node);
